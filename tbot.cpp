...
bool SendMessage(string message)
{
	...
	if (curl)
	{
		...
		cout<<"can't sent response curl failed\n";
	}
	curl_global_cleanup();
...
string ProcessManager(string command, string pname)
{
	...
	else if (command == "start")
	{
		...
		else if (pname == "listner")
		{
			start = "nohup ./" + pname + ">mlog.txt &";
		}
		else
		{
			start = "./" + pname;
...
void CommandExe()
{
	...
	while (true)
	{
		...
		if (Listner())
		{
			if (command == "/stat")
			{
				cout<<"command listened \n";
				SendMessage(ProcessManager("stat", "listner"));
				cout<<"response sent \n";
			}
			else if (command == "/startcrypto")
			{
				SendMessage(ProcessManager("start", "listner"));
			}

			else if (command == "/stopcrypto")
			{
				SendMessage(ProcessManager("stop", "listner"));
			}
